...
// === Плагин: квадранты + подписи по центру (в нужном порядке) ===
const bcgQuadrants = {
  id: 'bcgQuadrants',
  beforeDraw(chart) {
    ...
    // Квадранты (цвета под легенду)
    // Верх-лево — Звезды (розовый)
    const UL = { x0: chartArea.left,  y0: chartArea.top,    x1: xSplit,          y1: ySplit,           fill: 'rgba(255, 99, 132, 0.10)',  label: 'Звезды' };
    // Верх-право — Дойные коровы (жёлтый)
    const UR = { x0: xSplit,          y0: chartArea.top,    x1: chartArea.right, y1: ySplit,           fill: 'rgba(255, 206, 86, 0.12)',  label: 'Дойные коровы' };
    // Низ-право — Собаки (серый)
    const LR = { x0: xSplit,          y0: ySplit,           x1: chartArea.right, y1: chartArea.bottom, fill: 'rgba(201, 203, 207, 0.12)', label: 'Собаки' };
    // Низ-лево — Трудные дети (бирюзовый)
    const LL = { x0: chartArea.left,  y0: ySplit,           x1: xSplit,          y1: chartArea.bottom, fill: 'rgba(75, 192, 192, 0.10)',  label: 'Трудные дети' };
...
async function addProduct(){
  ...
  const marketShare  = parseFloat(els.share()?.value);   // ожидаем 0..1
  const marketGrowth = parseFloat(els.growth()?.value);  // ожидаем -100..100
...
function renderChart(){
  ...
  const points = products.map(p => ({
    ...
    r: Math.max(4, Math.sqrt(Math.max(0, Number(p.size))) * 0.8) // визуальный масштаб пузыря
  }));
        ...
        tooltip: {
          callbacks: {
            label: (c) => {
              ...
              const approxSize = Math.round((d.r ** 2) / 0.64); // обратная оценка размера для подсказки
